...
static NSInteger ftui_controller_seed = 0;

@interface FTUIShortEngine ()

@property(nonatomic, strong) NSMutableDictionary<NSNumber *, FTUIShortController *> *controllers;
...
- (NSNumber *)createShortController {
  @synchronized(self.controllers) {
    NSNumber *controllerId = @(ftui_controller_seed++);
    FTUIShortController *controller =
        [[FTUIShortController alloc] initWithControllerId:controllerId.unsignedIntegerValue
                                                observer:self];
