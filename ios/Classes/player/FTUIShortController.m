...
- (nonnull instancetype)initWithControllerId:(NSUInteger)controllerId observer:(nonnull id<FTUIShortEngineObserver>)observer {
    self = [super init];
...
- (void)bindVideoView:(NSInteger)pageViewId index:(NSInteger)index isPreBind:(BOOL)isPreBind {
    ...
    FTUIShortVideoView *itemView = [FTUIViewRegistry viewForTag:@(pageViewId)];
    if (nil != itemView) {
