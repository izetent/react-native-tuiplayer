...
public class FTUIShortController implements TUIPlayerBridge {
  ...
  private final FTUIShortEngineObserver engineObserver;
  private final int controllerId;
  private int currentIndex;
  private boolean isLoop = true;

  public FTUIShortController(Context context, int controllerId, FTUIShortEngineObserver observer) {
  ...
  private void bindVideoViewInternal(int viewTag, int index, boolean isPreBind) {
    UiThreadUtil.runOnUiThread(
        () -> {
          FTUIShortVideoItemView itemView = FTUIViewRegistry.get(viewTag);
          if (itemView == null) {
